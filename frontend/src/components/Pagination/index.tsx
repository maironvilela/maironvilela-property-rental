...
export default function Pagination({ totalPage }: PaginationProps) {
  const isDisplayInformationPage = useBreakpointValue({ base: true, sm: false })
  ...
  return (
    ...
    >
      ...
      {isDisplayInformationPage ||
        <Box>
          <Text>Pagina {currentPage} de {totalPage}  </Text>
        </Box>}

      <HStack spacing="1" ml="auto">

        {pages.map(page => (
          <PaginationItem disable={currentPage == page} page={page} key={page} />
